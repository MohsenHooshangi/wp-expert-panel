<div class="dashboard_container_header">
    <div class="dashboard_fl_1">
        <h4>تنظیمات حساب کاربری</h4>
    </div>
</div>
<div class="dashboard_container dashboard_container_body  p-4">
    <!-- Basic info -->
    <div class="submit-section">
        <div class="form-row">
<?php

?>
            <div class="form-group col-md-6">
                <label>نام کامل</label>
                <input type="text" class="form-control" value="<?php echo $current_user_display_name ?>">
            </div>

            <div class="form-group col-md-6">
                <label>ایمیل</label>
                <input type="email" class="form-control" value="<?php echo $current_user_email ?>">
            </div>

            <div class="form-group col-md-6">
                <label>مقطع آموزش</label>
                <input type="text" class="form-control" value="برنامه نویس وب">
            </div>

            <div class="form-group col-md-6">
                <label>شماره تماس</label>
                <input type="text" class="form-control" value="123 456 5847">
            </div>

            <div class="form-group col-md-6">
                <label>آدرس</label>
                <input type="text" class="form-control" value="تهران، سعادت آباد، خیابان سرو غربی">
            </div>

            <div class="form-group col-md-6">
                <label>استان</label>
                <input type="text" class="form-control" value="تهران">
            </div>

            <div class="form-group col-md-6">
                <label>شهرستان</label>
                <input type="text" class="form-control" value="کرج">
            </div>

            <div class="form-group col-md-6">
                <label>کدپستی</label>
                <input type="text" class="form-control" value="160052">
            </div>

            <div class="form-group col-md-12">
                <label>درباره من</label>
                <textarea class="form-control">زمان مورد نیاز شامل حروفچینی دستاوردهای اصلی و جوابگوی سوالات پیوسته اهل دنیای موجود طراحی اساسا مورد استفاده قرار گیرد.</textarea>
            </div>

        </div>
    </div>
    <!-- Basic info -->

    <!-- Social Account info -->
    <div class="form-submit">
        <h4 class="pl-2 mt-2">اکانت های اجتماعی</h4>
        <div class="submit-section">
            <div class="form-row">

                <div class="form-group col-md-6">
                    <label>Facebook</label>
                    <input type="text" class="form-control" value="https://facebook.com">
                </div>

                <div class="form-group col-md-6">
                    <label>Twitter</label>
                    <input type="email" class="form-control" value="https://twitter.com">
                </div>

                <div class="form-group col-md-6">
                    <label>Google Plus</label>
                    <input type="text" class="form-control" value="https://googleplus.com">
                </div>

                <div class="form-group col-md-6">
                    <label>LinkedIn</label>
                    <input type="text" class="form-control" value="https://linkedin.com">
                </div>

                <div class="form-group col-lg-12 col-md-12">
                    <button class="btn btn-theme" type="submit">ذخیره تغییرات</button>
                </div>

            </div>
        </div>
    </div>
    <!-- / Social Account info -->

</div>